...
module.exports = {
    ...
    async execute(interaction:ChatInputCommandInteraction, client:Client) {
        ...
        let userf = {
            ...
            namePrefix: '',
            nameSuffix: '',
            roles: (member.roles.cache.sort((a, b) => b.position - a.position).map(role => role.toString()).slice(0, -1)),
        ...
        const formatting = {
            ...
            flags: {
                ...
                'VerifiedDeveloper'     : "<:early_verified_bot_developer:1010557509938982933>"
            },
            'userStatus': {
        ...
        if (user.bot && client.config.alka.bots.includes(user.id) && !userf.namePrefix.includes(client.icon.alka.bot)) { // check if user is bot, if yes check if user.id is included in config.alka.bot, if true check if it doesnt have the badge on the prefix already, if not add one.
            userf.namePrefix = userf.namePrefix + ' ' + client.icon.alka.bot;
        };
        if (user.id === interaction.guild?.ownerId) { // check if user is server owner, if yes add server owner badge
            userf.namePrefix = userf.namePrefix + ' ' + client.icon.server.owner;
        }; 
