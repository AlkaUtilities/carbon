...
module.exports = {
    ...
    async execute(interaction:ChatInputCommandInteraction, client:Client) {
        ...
        if (channel.rateLimitPerUser === 0) return interaction.reply({ content: "Slowmode is already disabled." })
        ...
        let response = `${duration ? `Slowmode has been disabled for ${ms(duration, { long: true })}` : `Slowmode has been disabled`}\nReason: ${reason}`
        ...
        if (duration && previousRate > 0)
            setTimeout(async () => {
                ...
                interaction.followUp({ content: `Slowmode has been enabled with a rate of ${previousRate} per message`, fetchReply: true });
            }, duration);
    }
}