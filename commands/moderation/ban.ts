    ...
    async execute(interaction:ChatInputCommandInteraction) {
        ...
        if (targetMember !== undefined) { 
            ...
            if (targetRoles.highest.position >= memberRoles.highest.position && (targetRoles.cache.size === 0 && memberRoles.cache.size === 0 && interaction.guild?.ownerId === interaction.user.id)) {
                ...
                return await interaction.followUp({ content: "Unable to take action on this user as user isn't bannable.", ephemeral: true })
            ...
            if (bans?.find(m => m.user.id === target.id)) return interaction.followUp({ content: `**${target.tag}** is already banned.` })
