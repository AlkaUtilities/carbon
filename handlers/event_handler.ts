...
async function load_events(client:Client) {
    ...
    for (const file of files) {
        ...
        if (!event.name) { // check if file has property "name"
            table.push([ event.friendlyName ? event.friendlyName : event.name ? event.name : file.split('/').pop() , config.cli.status_bad ]);
            invalidEvents++
        ...
        table.push([ event.name ? event.name : file , config.cli.status_ok ]);
    };
